...
- A cross-device move that fails part way no longer leaves its half-copy in the trash, and no longer retries the same copy under a new name up to 1000 times. Measured with a 20 MB trash volume, `rm -r` of a 41 MB directory used to end with that volume 100% full of a 20 MB half-copy nothing mentioned, under a message blaming a name collision; it now removes the debris, stops, and says the trash filesystem may be out of space and that the source was not touched. The removal requires the move to have failed, the source to still be the same inode, and the target to be under the trash root; otherwise the previous behaviour stands.
