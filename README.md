...
- **強制覆蓋**：如果您希望直接覆蓋而不顯示提示，可以加上 `-f` 參數：
  ...
  ```
- **覆蓋是以 rename 原子取代**：還原時，垃圾桶項目會先搬進一個本行程獨佔建立的暫存目錄，再用同一檔案系統內的一次 rename 就位；既有的檔案或符號連結由核心在 rename 當下解除連結，過程中不存在「先把目的地刪掉」的步驟。跨裝置複製也發生在暫存目錄裡，複製失敗時目的地完全沒被碰過。
- **`--restore` 的結束碼**：
  - `0` — 還原完成，且沒有動到任何非預期的東西。
  - `1` — 還原沒有發生。狀態維持呼叫前的樣子；若有東西無法放回原位，錯誤訊息會直接指出它實際在哪裡。
  - `2` — **部分成功**：檔案確實還原了，但收尾（刪除被讓位的舊目錄、或移除暫存目錄）因為偵測到並行修改而被拒絕執行。殘留物的路徑會印在訊息裡，請自行檢查後處理。這種情況絕不會回傳 `0`。
