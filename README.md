...
`--` 之後的每一個引數都是檔名。沒有 `--` 的話 `-dash.txt` 會被當成選項而被拒絕，這與 `rm(1)` 一致；還原時同樣要寫 `rm --restore -- -dash.txt`。
...
- **名稱以破折號開頭時要用 `--`**：`rm --restore -- -dash.txt`。與刪除端一樣，沒有 `--` 的破折號開頭引數會被視為「`--restore` 缺少引數」，以免把誤打的 `rm --restore -f` 默默當成要還原的檔名。
- **重名保護機制**：若目前資料夾已存在同名檔案或目錄，`better-rm` 會提示您是否確認覆蓋（`y/N`）。
- **強制覆蓋**：如果您希望直接覆蓋而不顯示提示，可以加上 `-f` 參數：
  ```bash
  rm -f --restore LICENSE
  ```
- **覆蓋是以 rename 原子取代**：還原時，垃圾桶項目會先搬進一個本行程獨佔建立的暫存目錄，再用同一檔案系統內的一次 rename 就位；既有的檔案或符號連結由核心在 rename 當下解除連結，過程中不存在「先把目的地刪掉」的步驟。跨裝置的複製發生在暫存目錄裡，複製失敗時目的地完全沒被碰過。若目的地起初不存在、卻在還原途中才出現，未經覆蓋同意的還原會中止並把垃圾桶項目放回，而且那個目的地連一次 mv 的落點都不會是。
- **rename 換不掉的目的地，會被「移進垃圾桶」而不是被刪掉**：有兩種情況 rename 無法原子取代目的地 —— 目的地是真目錄，或還原的項目是真目錄而目的地不是（`rename(目錄, 非目錄)` 是 `ENOTDIR`）。這兩種情況下，舊的目的地會用本工具自己的垃圾桶機制保存起來（含刪除日誌紀錄），訊息會直接告訴您可以用 `rm --restore <名稱>` 取回它。`--restore` 全程不會執行任何遞迴刪除。
  - 反過來說：當 rename **可以**原子取代目的地時（兩邊都不是真目錄），舊的目的地就由核心解除連結而消失，與 `mv -f` 的行為一致，不會進垃圾桶。
...
- **`--restore` 的結束碼**：
  - `0` — 還原完成，且沒有動到任何非預期的東西（被 rename 換掉或被移進垃圾桶的舊目的地不算「非預期」，訊息都會講）。
  - `1` — 還原沒有發生。錯誤訊息會直接指出任何無法放回原位的東西實際在哪裡；跨裝置複製若已經消耗掉垃圾桶來源，訊息會印出資料目前所在的暫存路徑。
  - `2` — **部分成功**：項目確實還原了，但收尾（移除暫存目錄）沒能完成。殘留物的路徑會印在訊息裡，請自行檢查後處理。這種情況絕不會回傳 `0`。
