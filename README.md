...
**2026-09-05 再補正：xargs 的短選項會「串在一起」，而串起來才是一般寫法。** 上面那張表修好了
「每個選項吃幾個字」，但只在選項單獨成字時有效。`-0I{}` 是一個字裡有兩個選項，走訪沒有把它
拆開，於是替換字串沒被記下、後面的 `-c '{}'` 看起來像寫在命令列上的腳本，
`echo 'rm -rf /etc' | xargs -0I{} sh -c '{}'` 就被放行了——而完全分開寫的 `-0 -I{}` 會被擋，
這個不對稱正是線索。現在照 POSIX 的選項串文法拆：串裡每個字元都是一個選項，第一個「要值」
的字元把串的剩餘部分當成它的值，串到此為止時就吃下一個字（本機實測佐證：`xargs -0P2I{}` 回
`xargs: -P 2I{}: invalid`，`xargs -0EX I{} sh -c '{}'` 會去執行 `I{}`）。本機（BSD xargs）用
touch marker 在三個 shell 實測，`-0I{}`、`-tI{}`、`-rI{}`、`-t0I{}`、`-rtI{}`、`-0I %`、
`-0J%`、`-p0I{}`、`-tp0I{}`、`-0I{} -t`、`-0E EOF bash -c`、`-0P 4 bash -c` 全都真的執行了從
stdin 進來的 payload；`-0i{}` 與 `-xI{}` 在本機跑不起來（BSD 拒絕 `-i`、`-x` 需搭配 `-n`/`-s`），
但 GNU findutils 兩個都收，而本專案有出貨到 Linux，所以照擋。刻意的過度拒絕一條：串尾的
`-0I` 只在「後面還留著命令字」時才吃下一個字，否則 `xargs -0I rm -rf` 這種行（真的 xargs 會
回 `invalid option -- f`，什麼都不會跑）會從拒絕變成放行。剩下的代價見 KNOWN-RESIDUALS.md 的
R5-i。

**2026-09-05：heredoc 的「結束標記」改用與其他任何字相同的規則來讀。** 內文在哪裡結束，全由
這一個字決定，而內文結束錯地方，後面的一切就對本檔所有掃描隱形。原本的讀法不認 ANSI-C 引號
（`$'…'`）也不認行接續（`EO\`+換行+`F`），而二十行外的 tokenizer 兩者都認：`cat <<$'EOF'` 算
出來的標記是 `$EOF`，永遠對不上 `EOF` 那一行，內文一路吃到輸入結尾，又因為標記裡有引號而被
當成「字面」抹掉——heredoc 後面那條 `rm -rf /etc` 就此隱形，整條命令放行。2026-09-05 用 touch
marker 在 /bin/bash 3.2.57、/opt/homebrew/bin/bash 5.3.15 與 /bin/sh 三個 shell 實測，十二種
拼法（`$'EOF'`、`$'E\x4FF'`、`$'EO'F`、`$'E\tF'`、`<<-$'EOF'`、`$"EOF"`、`EO\`+換行+`F`、
`"EO\`+換行+`F"`、`"E\$F"`、`$'E\qF'`、`$(echo A)`、`` `echo A` ``）的 marker 全都出現。
「有沒有加引號」也一併照 bash 的規則重讀：`$'…'`／`$"…"`／`"…"`／`'…'`／`\E` 讓內文變字面，
而行接續是在「問這個字有沒有加引號」之前就被刪掉的，所以 `EO\`+換行+`F` 的內文是會展開的
（舊讀法把它算成加了引號，是同一個繞過的另一半）。反引號那種是兩個 bash 唯一分歧之處
（3.2 字面、5.3 展開），取「未加引號」這一邊，也就是掃得比較多的那一邊。
**新的拒絕一種**：結束標記的引號到輸入結尾都沒有收尾時（`cat <<'EOF` ——bash 自己也讀不了，
三個 shell 實測都是語法錯誤），整條命令以 `unreadable heredoc delimiter` 拒絕，而不是把命令
剩下的部分整段當成標記、讓內文吃到輸入結尾。
